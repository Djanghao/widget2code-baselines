...
# Canvas size — YOU pick it (only aspect ratio is given)

You will only receive the widget's **aspect ratio** (e.g. "≈1.00:1"), NOT an exact pixel size. You must choose a suitable canvas `width` and derive `height = round(width / aspect_ratio)`. Set both as explicit px on the root `<div className="widget">`.

**Canvas-width selection rubric** — pick by complexity, NOT by how big the widget "should" look:

| Complexity | Chosen width | Use when the screenshot shows… |
|------------|-------------:|--------------------------------|
| **Trivial** | **300 px** | 1–3 elements total (single icon + label, one metric badge) |
| **Simple** | **500 px** | 3–7 elements, one section (a small card, avatar+title+sub) |
| **Standard** | **700 px** | A card with 4+ rows, or a 2-column layout with ~6 cells |
| **Rich** | **900 px** | Header + body + footer, or image + side info, or 3×2 grid |
| **Complex** | **1100 px** | Dashboard, multi-section, embedded chart, list + sidebar |
| **Extreme** | **1400 px** | Only for widgets with dense charts + many rows + legend |

**Default to 700** if you are uncertain — it gives enough room for typical card/list widgets without wasting canvas on trivial ones.

Derive height from the given aspect ratio:
- aspect ratio "≈1.00:1" → square → `width=700, height=700`
- aspect ratio "≈2.00:1" → wide banner → `width=900, height=450`
- aspect ratio "≈0.50:1" → tall (portrait) → `width=500, height=1000`

# Internal sizing philosophy (all px, proportional to YOUR canvas)

Once you've picked canvas `(W, H)`, every interior dimension is measured **relative to YOUR canvas**, NOT relative to the screenshot's raw pixels:

- `fontSize` for main title: roughly **8–12% of W** (e.g. W=700 → title ≈ 56–84 px; W=400 → 32–48 px).
- `fontSize` for body text: roughly **4–6% of W**.
- `fontSize` for small labels / captions: roughly **3–4% of W**.
- `borderRadius` for widget shell: roughly **3–8% of W** (iOS widgets look "big-corner" at ~6%).
- `padding` for widget shell: roughly **4–8% of W**.
- `gap` between cards/rows: roughly **2–5% of W**.
- Icon size for decorative glyphs: roughly **5–10% of W**.

These are ratios to help you pick — write the final values as **concrete px** (e.g. `fontSize: '56px'`), NEVER as `%` or `calc()`.

**NEVER use:** `%`, `rem`, `em`, `vw`, `vh`, `calc()` for sizing. `min-width`, `max-width`, `aspect-ratio` (css), media queries, `clamp()`. Every dimension is concrete px.

Internal % exception: `width: '100%'` or `width: 'X%'` inside a fill-the-row child of a flex parent (rare; usually `flex: 1` is preferred).
...
The root widget has an explicit `height` you computed from the aspect ratio. When it contains stacked sections (2+ cards, header+body, header+body+footer), the sum of section heights should roughly fill the widget.
...
# Color palette (bench analysis of the target image)

The block below describes the target screenshot's color character using the **same HSV-histogram + polarity analysis that the evaluation uses**. Align your widget's color choices to match each field as closely as possible — this is the highest-leverage way to maximize palette / vibrancy / polarity scores.

[COLOR_PALETTE]

**How to use this block**:

- **Theme polarity** (`light-themed` vs `dark-themed`): this determines whether the widget's root background must be dark or light. Do NOT flip the polarity — a dark-themed screenshot rendered as a light widget scores 0 on `PolarityConsistency`.
- **Saturation profile**: tells you the overall "colorfulness" budget.
  - `desaturated / mostly neutral` → use grays, off-whites, low-sat tints. Keep any accent color to 1-2 small areas.
  - `mixed saturation` → neutral bases + 1-2 vivid accent colors (buttons, progress bars, highlights).
  - `highly-saturated / vivid` → bold saturated blocks are core to the design; don't render it as neutral gray.
- **Hue variety**: tells you how many distinct color families to allow.
  - `monochromatic` → one dominant hue + grays only.
  - `few hues (2-3 accents)` → 2-3 accent hues allowed.
  - `multi-hue / colorful` → multiple distinct hues are required.
- **Dominant colors** (k-means hex list): use these as the actual hex values in `backgroundColor`, `color`, `stroke`, `fill`. Every color you write should be one of these or within ~10 RGB units of one. Do NOT invent hues outside this palette.
...
# Size-tier discipline (based on YOUR chosen canvas width)

Match code complexity to the width you chose:

- **W = 300 (Trivial)**: ≤3 elements, no padding, nesting ≤3 levels.
- **W = 500 (Simple)**: 1–2 sections, padding 16–40.
- **W = 700 (Standard)**: header/body/footer, padding 40–60, `items.map()` for lists.
- **W = 900 (Rich)**: multi-region with dividers, padding 50–80, may use recharts.
- **W = 1100–1400 (Complex/Extreme)**: `const data` at module level, custom recharts shape components, dense layouts.
