...
The list below is every text string detected by OCR in the screenshot. Positions are **fractional** (percent of widget width/height) — since your `<div className="widget">` matches the screenshot's pixel size, multiply the percentages by that W/H to get concrete px.
...
- **Font size**: the `font-height ≈ N% of widget height` tells you how tall the glyph is relative to the widget's height. If the widget is `H = 800 px` and OCR says the title glyph is `~12% of height`, use `fontSize: '96px'` (800 × 0.12). Clamp to reasonable bounds (≥ 10 px for small labels).
