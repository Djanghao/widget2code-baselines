...
# Text content (from OCR of the target image)

The list below is every text string detected by OCR in the screenshot. Positions are **fractional** (percent of widget width/height) so they work at any canvas size you pick.

[OCR_TEXT]

**How to use**:

- **Copy every string exactly as written** — do NOT rewrite, translate, abbreviate, or re-case. `"Nothing Scheduled for Today"` must render as `"Nothing Scheduled for Today"`, not `"No events"`. `"09:15 - 10:15 AM"` stays verbatim, not `"9:15am-10:15am"`.
- **Spatial ordering**: use the `(x%, y%)` to place text in the correct layout region. Texts with the same `y%` (±2–3%) belong to the same row; texts with small `x%` (e.g. `x% < 20`) are typically in a left column / sidebar.
- **Font size**: the `font-height ≈ N% of widget height` tells you how tall the glyph is relative to your widget's height. If your canvas is `H = 500 px` and OCR says the title glyph is `~12% of height`, use `fontSize: '60px'` (500 × 0.12). Clamp to reasonable bounds (≥ 10 px for small labels).
- If the OCR list shows text that isn't actually in the screenshot (false positives from decorative strokes), ignore it. If the screenshot has obvious text not in the list, still render it — OCR isn't exhaustive.
- Do not emit any text that is not in either the OCR list or clearly visible in the screenshot.
